...
export function apiUnreachableError(cause: unknown): Error {
  ...
  return new Error(
    `Cannot reach the CourseStack API at ${API_URL}. Start the backend: \`cd backend && uvicorn main:app --reload --port 8000\`, or set NEXT_PUBLIC_API_URL in frontend/.env.local. (${detail})`,
  );
}
